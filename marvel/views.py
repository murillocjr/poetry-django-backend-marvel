    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            if serializer.data["username"] == "admin" and serializer.data["password"] == "123456":
                return Response({"status": "success", "code": 200, "data": { "username": "user-user", "devicename": "android", "genericname": "demo"} }, status=status.HTTP_200_OK, content_type="application/json")
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
